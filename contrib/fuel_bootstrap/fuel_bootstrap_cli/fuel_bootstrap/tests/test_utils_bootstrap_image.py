...
FAKE_OS = [('/test',
            ['/test/image_1', '/test/image_2',
             '/test/link_active_bootstrap'], ['/test/test_file']),
...
def _exists(dir_path):
    for roots, dirs, files in FAKE_OS:
        for d in dirs:
            if d == dir_path:
                return True
        for f in files:
...
class BootstrapImageTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.conf_mock.bootstrap_images_dir = '/test'
        self.conf_mock.active_bootstrap_symlink = '/test/link_active_bootstrap'
    ...
    @mock.patch.object(bs_image, 'is_active', autospec=True)
    def test_parse_correct_image(self, active_mock):
        active_mock.return_value = False
        image_uuid = '/test/image_1'
        expected_data = {'uuid': 'image_1'}
        self.yaml_mock.return_value = expected_data
    ...
    def test_parse_active_image(self, active_mock):
        ...
        image_uuid = '/test/image_1'
        expected_data = {'uuid': 'image_1', 'label': ''}
        self.yaml_mock.return_value = expected_data
        expected_data.update({'label': '', 'active': ''})
        data = bs_image.parse(image_uuid)
        self.assertEqual(expected_data, data)

    @mock.patch.object(bs_image, 'parse', autospec=True)
    ...
    def test_full_path_not_full(self):
        ...
        self.assertEqual('/test/image_1', result)
    ...
    @mock.patch('tempfile.mkdtemp', return_value='/tmp/test', autospec=True)
    @mock.patch('fuel_bootstrap.utils.bootstrap_image.extract_to_dir',
                autospec=True)
    def test_import_image(self, extract_mock, tempfile_mock, shutil_mock,
        ...
        chmod_mock.assert_has_calls([mock.call(dir_path, 0o755),
                                     mock.call(dir_path + '/directory', 0o755),
                                     mock.call(dir_path + '/file', 0o755)])
    ...
    @mock.patch.object(fuel_agent.manager, 'Manager', autospec=True)
    ...
    @mock.patch('fuel_agent.utils.utils.execute', autospec=True)
    ...
    @mock.patch.object(bs_image, '_update_astute_yaml', autospec=True)
    @mock.patch.object(bs_image, '_run_puppet', autospec=True)
    @mock.patch.object(fuel_agent.utils.utils, 'execute', autospec=True)
    ...
    @mock.patch('os.path.lexists', return_value=False, autospec=True)
    ...
    @mock.patch('os.path.lexists', return_value=True, autospec=True)
    @mock.patch('os.unlink', autospec=True)
    @mock.patch('os.symlink', autospec=True)
    ...
    @mock.patch.object(bs_image, '_activate_flavor', autospec=True)
    ...
    @mock.patch.object(bs_image, 'parse', autospec=True)
    @mock.patch.object(bs_image, '_activate_image', autospec=True)
