...
class BootstrapImageTestCase(unittest2.TestCase):
    ...
    def test_run_puppet_cobbler_manifest(self, execute_mock):
        manifest = consts.COBBLER_MANIFEST
        bs_image._run_puppet(manifest)
        execute_mock.assert_called_once_with('puppet', 'apply',
                                             '--detailed-exitcodes',
    ...
    def test_run_puppet_astute_manifest(self, execute_mock):
        manifest = consts.ASTUTE_MANIFEST
        bs_image._run_puppet(manifest)
        execute_mock.assert_called_once_with('puppet', 'apply',
                                             '--detailed-exitcodes',
                                             '-dv', manifest, logged=True,
    ...
    def test_activate_image(self,
                            ...
                            conf_mock):
        image_uuid = '/test/test_image_uuid'
        os_mock.path.lexists.return_value = False
    ...
    def test_activate_image_symlink_deleted(self,
        ...
        os_mock.path.lexists.return_value = True
        self.assertEqual(image_uuid, bs_image._activate_image(image_uuid))
