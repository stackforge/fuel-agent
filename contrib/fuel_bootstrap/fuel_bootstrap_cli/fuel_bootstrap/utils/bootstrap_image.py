...
def _cobbler_system_edit_default(profile=None):
    """Change default cobbler system profile

    :param profile: change default syste to this profile
    :return:
    """
    LOG.debug('Changing default cobbler system profile to:%s', profile)
    check_regex = re.compile('Profile\s*:\s*{0}'.format(profile))
    cmd = ['dockerctl', 'shell', consts.COBBLER_DOCKER,
           'cobbler', 'system', 'report', '--name=default']
    if check_regex.match(utils.execute(*cmd)[0]) is None:
        utils.execute('dockerctl', 'shell', consts.COBBLER_DOCKER,
                      'cobbler', 'system', 'edit', '--name=default',
                      '--profile={0}'.format(profile),
                      '--netboot-enabled=True', logged=True, attempts=2)
    utils.execute('dockerctl', 'shell', consts.COBBLER_DOCKER,
                  'cobbler', 'sync', logged=True, attempts=2)
...
def _update_astute_yaml(flavor=None):
    ...
    try:
        ...
        LOG.error("File %s has not been opened successfully", config)
        raise
...
def _activate_dockerized(flavor=None):
    ...
    if flavor not in consts.DISTROS:
        raise errors.WrongCobblerProfile(
            ...
            flavor.lower(), consts.DISTROS.keys())
    _update_astute_yaml(consts.DISTROS[flavor]['astute_flavor'])
    _cobbler_system_edit_default(consts.DISTROS[flavor]['cobbler_profile'])
