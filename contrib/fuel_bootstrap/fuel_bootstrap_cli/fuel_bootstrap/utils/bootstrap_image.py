...
from fuelmenu.fuelmenu import Settings as fm_conf
...
def _update_astute_yaml(flavor=""):
    LOG.debug("Switching in %s ['BOOTSTRAP']['flavor'] to :%s",
              consts.ASTUTE_CONFIG_FILE, flavor)
...
def _run_puppet(container="", manifest=""):
...
def _activate_dockerized(flavor=""):
    ...
    1) Update asute.yaml- which used by puppet to determinate option's
