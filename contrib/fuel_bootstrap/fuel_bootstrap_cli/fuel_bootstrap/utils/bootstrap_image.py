...
def _cobbler_system_edit_default(flavor=None):
    """Change default cobbler system profile

    :param flavor:
    :return:
    """
    check_regex = re.compile('Profile\s*:\s*{0}'.format(flavor))
    ...
    if check_regex.match(utils.execute(*cmd)[0]) is None:
        utils.execute('dockerctl', 'shell', consts.COBBLER_DOCKER,
                      ...
                      '--profile={0}'.format(flavor), '--netboot-enabled=True',
                      logged=True, attempts=2)
...
def _update_astute_yaml(flavor=None):
    ...
    try:
        ...
        LOG.debug("File %s has not been opened successfully", config)
        raise
...
def _run_puppet(container=None, manifest=None):
    ...
    utils.execute('dockerctl', 'shell', container, 'puppet', 'apply',
                  '--detailed-exitcodes', '-dv', manifest, '2>&1', logged=True,
...
def _activate_dockerized(flavor=None):
    ...
    if flavor.lower() not in consts.DISTROS:
