...
def get_all():
    ...
    for name in os.listdir(consts.BOOTSTRAP_IMAGES_DIR):
        try:
            ...
            LOG.debug("[{0}] image skipped because of {1}"
                     .format(name, e))
    return data


def parse(image_id):
    LOG.debug("Trying to parse [{0}] image".format(image_id))
    dir_path = full_path(image_id)
    if os.path.islink(dir_path) or not os.path.isdir(dir_path):
        raise errors.IncorrectImage("There are no directory for image [{0}]."
                                    .format(image_id))
    ...
    with open(metafile) as f:
        try:
            ...
            raise errors.IncorrectImage("Couldn't parse metadata file for"
                                        " image [{0}] because of {1}"
                                        .format(image_id, exc))
    if data.get('uuid') != os.path.basename(dir_path):
        raise errors.IncorrectImage("UUID from metadata file [{0}] doesn't equal"
                                    " directory name [{1}]"
...
def delete(image_id):
    ...
    if image['status'] == ACTIVE:
        raise errors.ActiveImageException("Image [{0} is active and can't"
                                          " be deleted.".format(image_id))
...
def extract_to_dir(arch_path, extract_path):
    LOG.info("Try extract {0} to {1}".format(arch_path, extract_path))
