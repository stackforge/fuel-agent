...
def set_default_fields(data):
    data['id'] = data.get('uuid', six.text_type(uuid.uuid4()))
    data['label'] = data.get('label', data['id'])
    data['status'] = 'active' if is_active(data['id']) else ''
    data['extend_kopts'] = data.get('extend_kopts', '')
...
def activate(image_id):
    ...
    try:
        ...
        LOG.warning("Symlink {0} can't be removed because it doesn't exists"
...
def import_image(arch_path):
    ...
    os.rename(extract_dir, full_path(image_id))
