...
class BootstrapDataBuilder(object):
    ...
    def __init__(self, kwargs):
        ...
        self.ubuntu_release = kwargs.ubuntu_release
    ...
    def _get_direct_repo_addr(self, direct_repo_addr):
        addrs = set()
        if direct_repo_addr:
            addrs |= set(direct_repository)
    ...
    def _get_repos(self):
        result = []
        if self.ubuntu_repo:
            result.extend(self._parse_ubuntu_repos(self.ubuntu_repo))
        else:
            result.extend(self.astute['BOOTSTRAP']['MIRROR_DISTRO'])

        if self.mos_repo:
            result.extend(self._parse_mos_repos(self.mos_repo))
        else:
            result.extend(self.astute['BOOTSTRAP']['MIRROR_MOS'])

        repo_count = 0
        for repo in self.repos:
            repo_count += 1
            result.append(self.parse_repo(repo, name="extra_repo{0}".format(repo_count)))

        #TODO: sort result by priority

        return result

    def _parse_not_extra_repo(self, repo):
    ...
    def _parse_mos_repos(self, repo):
    ...
    def parse_repo(self, repo, name=None, priority=None):
        ...
        return {'name': name,
                'type': repo_type,
                'uri': repo_uri,
                'priority': repo_priority or priority,
                'suite': repo_suite,
