...
class Manager(object):
    ...
    def do_bootloader(self):
        ...
        block_devs = [parted.name for parted
                      in self.driver.partition_scheme.parteds]
        is_mutlipath = {device: hw.is_multipath_device(device)
                        for device in block_devs}
        # If there are no multipath devices on the node, we should not do
        # anything to prevent regression.
        if any(is_mutlipath.values()):
            # We need to explicitly whitelist each non-mutlipath device
            lvm_filter = utils.lvm_whitelist_filter(block_devs, is_mutlipath)
            ...
            bu.override_lvm_config(
                chroot,
                {'devices': {'global_filter': lvm_filter,
                             'preferred_names': CONF.mpath_preferred_names}},
    ...
    def do_mkbootstrap(self):
        ...
        try:
            ...
            bu.override_lvm_config(
                chroot,
                {'devices': {'preferred_names': CONF.mpath_preferred_names}},
                lvm_conf_path=CONF.lvm_conf_path)
