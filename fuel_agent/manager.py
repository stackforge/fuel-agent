...
class Manager(object):
    ...
    def dump_mkbootstrap_meta(self, metadata, c_dir, bootstrap_scheme):
        ...
        drop_data = {'modules': {}}
        for module in bootstrap_scheme.modules.__dict__.iteritems:
            ...
            drop_data['modules'][getattr(module, "name")] = {
                'raw_md5': raw_md5,
                'raw_size': raw_size,
                'file': fname,
                'uri': module.uri
    ...
    def do_mkbootstrap(self):
        ...
        try:
            ...
            bootstrap_scheme = self.driver.bootstrap_scheme
            metadata = {}
            ...
            if bootstrap_scheme.extra_files:
                for extra in bootstrap_scheme.extra_files:
                    bu.rsync_inject(extra, chroot)
            if bootstrap_scheme.root_ssh_authorized_file:
                LOG.debug('Put ssh auth file %s',
                          bootstrap_scheme.root_ssh_authorized_file)
            ...
            if bootstrap_scheme.post_script_file:
                bu.run_script_in_chroot(
                    chroot, bootstrap_scheme.post_script_file)
            # Save runtime_uuid into bootstrap
            bu.dump_runtime_uuid(bootstrap_scheme.uuid,
                                 os.path.join(chroot,
            ...
            bu.recompress_initramfs(
                chroot,
                compress=bootstrap_scheme.modules.initrd.compress_format)
            ...
            if not bu.stop_chrooted_processes(chroot, signal=signal.SIGTERM):
                if not bu.stop_chrooted_processes(
                    ...
                    raise errors.UnexpectedProcessError(
                        ...
                        chroot)
            bu.run_mksquashfs(
                chroot, os.path.join(c_dir, os.path.basename(
                    bootstrap_scheme.modules.rootfs.uri)),
                bootstrap_scheme.modules.rootfs.compress_format)
            self.dump_mkbootstrap_meta(metadata, c_dir, bootstrap_scheme)
