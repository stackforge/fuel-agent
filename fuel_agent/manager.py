...
class Manager(object):
    ...
    def _get_required_fs_size(self, src_files):
        def dir_size(d):
            s = 0
            for root, dirs, files in os.walk(d):
                s += sum(os.path.getsize(os.path.join(root, name))
                         for name in files + dirs)
            return s

        req_size = 0
        for f in src_files:
            req_size += os.path.getsize(f)
            if os.path.isdir(f):
                req_size += dir_size(f)

        # 10% of size for fs metadata
        req_size = int(req_size * 1.1)
        # round up to megabytes
        return (req_size - 1) / (1 << 20) + 1

    def _make_configdrive_image(self, src_files):
        req_size = self._get_required_fs_size(src_files)

        utils.execute('dd', 'if=/dev/zero', 'bs=1M', 'count=%d' % req_size,
                      'of=%s' % CONF.config_drive_path)
        fu.make_fs(
            fs_type='ext4',
            fs_options=' -b 4096 -F ',
            fs_label='config-2',
        ...
        try:
            fu.mount_fs('ext4', CONF.config_drive_path, mount_point)
    ...
    def _prepare_configdrive_files(self):
        ...
        utils.render_and_save(
            tmpl_dir,
            self.driver.configdrive_scheme.template_names('network_config'),
            self.driver.configdrive_scheme.template_data(),
            nc_output_path
        )
