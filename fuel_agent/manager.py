...
class Manager(object):
            ...
            while head != fs_mount:
                ...
                if head in mount_map:
                    ...
                    if os.path.exists(check_path):
                        LOG.debug('Path %s exists. Trying to move all files '
                                  'from there to %s', mount_map[fs_mount])
                        for f in os.listdir(check_path):
                            src = os.path.join(check_path, f)
                            dst = os.path.join(mount_map[fs_mount], f)
                            if preremove_dst and os.path.isdir(dst):
                                LOG.warning('Trying to remove existent '
                                            'destination: %s', dst)
                                shutil.rmtree(dst)
                            LOG.debug('Trying to move file from %s to %s',
                                      src, dst)
                            os.rename(src, dst)
                        break
                if head == '/':
    ...
    def umount_target_flat(self, mount_map):
        ...
        for _, mount_point in six.iteritems(mount_map):
