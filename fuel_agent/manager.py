...
class Manager(object):
    ...
    def dump_mkbootstrap_meta(self, metadata, file):
        ...
        for module in self.driver.data['bootstrap_modules']:
            fname = os.path.basename(self.driver.data['bootstrap_modules']
                                     [module]['uri'])
            fs_file = os.path.join(os.path.dirname(file), fname)
            try:
    ...
    def do_mkbootstrap(self):
        ...
        Expected input self.driver.data scheme:
{'bootstrap_modules': {'m_initrd': {'compress_format': 'xz',
                                    'uri': 'http://any/initrd.img'},
                       'm_kernel': {'uri': 'http://any/vmlinuz'},
                       'm_rootfs': {'compress_format': 'xz',
                                    'container': 'raw',
                                    'format': 'ext4',
                                    'ro_system': 'squashfs',
                                    'uri': 'http://any/root.squashfs'}},
 'codename': 'trusty',
 'container': 'tar.gz',
 'extend_kopts': 'ifnames=1',
 'extra_files': ['/usr/share/mk_bootstrap/files/trusty'],
 'image_data': {'/': {'container': 'raw',
                      'format': 'ext4',
                      'uri': 'http://any/root.squashfs'}},
 'meta_file': 'metadata.yaml',
 'output': '/root/bootstrap.tar.gz',
 'packages':  list of packages
 'post_script_file': '/root/hello.sh',
 'proxies': dict with proxies data, more info in bu.run_debootstrap,
 'repos': dict with repo, more info in
    drivers.nailgun.NailgunBuildImage.parse_operating_system(),
 'ssh_authorized_keys': '/root/.ssh/id_rsa.pub',
 'uuid': '750a4bfe-5d7b-4555-91d0-c162fe10a8c6'}
        ...
        try:
            ...
            if 'extra_files' in self.driver.data:
                    for extra in self.driver.data['extra_files']:
            ...
            self.dump_mkbootstrap_meta(
                metadata,
                os.path.join(c_dir, self.driver.data['meta_file']))
