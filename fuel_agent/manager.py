...
class Manager(object):
    ...
    def _prepare_cloudinit_config(self, target_dir):
    ...
    def do_inject_cloudinit_config(self):
        LOG.debug('--- Injecting cloudinit config '
                  '(do_inject_cloudinit_config) ---')
        root_fs = self.driver.partition_scheme.fs_by_mount('/')
        root = fu.mount_fs_temp(root_fs.type, str(root_fs.device))
        try:
            self._prepare_cloudinit_config(
    ...
    def do_provisioning(self):
        ...
        self.do_copyimage()
        self.do_inject_cloudinit_config()
        self.do_bootloader()
