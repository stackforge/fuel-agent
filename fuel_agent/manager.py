...
class Manager(object):
    ...
    def do_bootloader(self):
        ...
        if multipath_devs:
            ...
            # Disable mdadm autoscan (LP: 1562836)
#            utils.execute('chroot', chroot, 'apt-get', 'purge -y', 'mdadm', logged=True)
            utils.execute('chroot', chroot, 'apt-get', 'purge -y', 'mdadm', logged=True)
            utils.execute('chroot', chroot, 'sed -i s#/lib/systemd/systemd-udevd#'/lib/systemd/systemd-udevd --debug --children-max=32 --resolve-names=never > /dev/.udev.debug 2>&1 &'#g /usr/share/initramfs-tools/scripts/init-top/udev' , logged=True)
#            utils.makedirs_if_not_exists(os.path.join(chroot, 'etc/mdadm/'))
#            with open(os.path.join(chroot, 'etc/mdadm/mdadm.conf'), 'w') as f:
#                f.write(u'# Generated by fuel-agent during provisioning: \n')
#                f.write(u'CREATE owner=root group=disk mode=0660 auto=yes \n')
#                f.write(u'HOMEHOST <system> \n')
#                f.write(u'MAILADDR root \n')
#                f.write(u'AUTO -all \n')
#                f.write(u'AUTOSTART=false \n')
#                f.write(u'ARRAY <ignore> \n')
...
#        grub.append_kernel_params('root=UUID=%s ' % mount2uuid['/'])
        grub.append_kernel_params('root=UUID={0} {1} '.format(mount2uuid['/'], 'nomdmonisw=1 nomdmonisw nomdmonddf=1 nomdmonddf debug loglevel=6 udev.debug raid=noautodetect nomdadm noraid mdassemble=none udev.log-priority=debug console=ttyS0 '))
