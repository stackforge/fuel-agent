...
class TestSizes(unittest2.TestCase):
    ...
    def test_sizes(self):
        for hvalue, bvalue in self.test_cases:
            self.assertEqual(SIZES.convert(hvalue), bvalue)
    ...
    def test_sizes_unnecessary_whitespace(self):
        for hvalue, bvalue in self.test_cases:
            ...
            self.assertEqual(SIZES.convert(ws_hvalue), bvalue)

    def test_not_supported_unit(self):
        test_cases = ('15b', '16mb', '12MEGAGIGA', '42PETA', '12Bi', '15 b')

        for test in test_cases:
            with self.assertRaisesRegexp(KeyError, 'is not supported'):
                SIZES.convert(test)

    def test_unit_without_value(self):
        test_cases = ('B', '  MB ', ' kB', 'TiB')

        for test in test_cases:
            with self.assertRaisesRegexp(
                    KeyError, 'Unit without a value has been given'):
                SIZES.convert(test)
