...
class TestNailgunMockedMeta(unittest2.TestCase):
    ...
    def test_configdrive_scheme_set_environment(self, mock_lbd,
                                                mock_image_meta):
        data = copy.deepcopy(PROVISION_SAMPLE_DATA)
        expected_version = 'fake_version'
        data['ks_meta']['environment_version'] = expected_version
        mock_lbd.return_value = LIST_BLOCK_DEVICES_SAMPLE
        cd_scheme = nailgun.Nailgun(data).configdrive_scheme
        self.assertEqual(expected_version, cd_scheme.environment)
