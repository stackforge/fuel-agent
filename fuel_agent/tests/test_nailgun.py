...
class TestNailgunMockedMeta(unittest2.TestCase):
    def test_parse_operating_system_test_profiles(
        ...
        d = {'centos-x86_64': objects.Centos65,
             'centos7-x86_64': objects.Centos70,
             'ubuntu_1204_x86_64': objects.Ubuntu1204,
             'ubuntu_1404_x86_64': objects.Ubuntu1404,
             'unsupported_ubuntu_profile': objects.Ubuntu,
             'unsupported_profile': objects.OperatingSystem}
        for profile, obj in d.iteritems():
            p_data['profile'] = profile
            self.assertIsInstance(nailgun.Nailgun(p_data).get_os_by_profile(),
                                  obj)

    def test_parse_operating_system_image_meta(self, mock_lbd,
                                               mock_image_meta):
        mock_lbd.return_value = LIST_BLOCK_DEVICES_SAMPLE
        d = {'Centos65': objects.Centos65,
             'Centos70': objects.Centos70,
             'Ubuntu1204': objects.Ubuntu1204,
             'Ubuntu1404': objects.Ubuntu1404,
             'unknown': None}
        for release, obj in d.iteritems():
            mock_image_meta.return_value = {'os_release': release}
            ...
            if os:
                ...
                self.assertEqual('unknown', release)
