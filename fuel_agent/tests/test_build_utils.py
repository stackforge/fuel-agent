...
class BuildUtilsTestCase(unittest2.TestCase):
    ...
    @mock.patch.object(utils, 'makedirs_if_not_exists')
    @mock.patch.object(utils, 'execute')
    def test_make_targz(self, mock_exec, mock_makedirs):
        bu.make_targz('/test/path')
        mock_exec.assert_called_with('tar', '-czf', mock.ANY, '--directory',
                                     '/test/path', '.', logged=True)

    @mock.patch.object(utils, 'makedirs_if_not_exists')
    @mock.patch.object(utils, 'execute')
    def test_make_targz_with_name(self, mock_exec, mock_makedirs):
        bu.make_targz('/test/path', 'testname')
    ...
    def test_get_installed_packages(self, mock_exec):
        ...
        self.assertItemsEqual(expected_packages, packages)

    @mock.patch.object(utils, 'makedirs_if_not_exists')
    @mock.patch.object(utils, 'execute')
    @mock.patch.object(glob, 'glob',
                       side_effect=[['testfile1'], ['testfile2']])
    @mock.patch.object(shutil, 'copy')
    ...
    @mock.patch.object(utils, 'execute')
    @mock.patch.object(glob, 'glob',
                       side_effect=[['testfile1'], ['testfile1'],
                                    ['testfile2'], ['testfile2']])
    @mock.patch.object(shutil, 'copy')
    @mock.patch.object(bu, 'remove_files')
    ...
    @mock.patch.object(utils, 'execute')
    @mock.patch.object(shutil, 'move')
    @mock.patch.object(fu, 'mount_fs')
    @mock.patch.object(fu, 'umount_fs')
    @mock.patch.object(bu, 'stop_chrooted_processes')
    def test_run_mksquashfs(self, mock_stop_proc, mock_umount, mock_mount,
                            mock_move, mock_exec, mock_makedirs):
        bu.run_mksquashfs('/test/dst/dir')
        expected_mount_args = [
            mock.call('tmpfs', mock.ANY, '/test/dst/dir/mnt',
            ...
            mock.call(None, u'', '/test/dst/dir/mnt/dst', opts='bind')
        ]
        self.assertItemsEqual(expected_mount_args, mock_mount.call_args_list)
        expected_umount_args = [
            mock.call('/test/dst/dir/mnt/dst'),
            mock.call('/test/dst/dir/mnt/src'),
            mock.call('/test/dst/dir/mnt')
        ]
        self.assertItemsEqual(expected_umount_args,
                              mock_umount.call_args_list)
        expected_exec_args = [
            mock.call('chroot', '/test/dst/dir', 'mksquashfs', '/mnt/src',
                      mock.ANY, '-comp', 'xz', '-no-progress', '-noappend',
                      logged=True)
        ]
        self.assertItemsEqual(expected_exec_args,
    ...
    @mock.patch.object(fu, 'umount_fs')
    @mock.patch.object(bu, 'stop_chrooted_processes')
    def test_run_mksquashfs_with_name(self, mock_stop_proc, mock_umount,
                                      mock_mount, mock_move, mock_exec,
                                      mock_makedirs):
        bu.run_mksquashfs('/test/dst/dir', output_name='myname')
        mock_move.assert_called_with(mock.ANY, 'myname')
