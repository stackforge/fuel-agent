...
foo {
\tbar=1
}''', bu._update_lvm_raw_config('foo', 'bar', 1, RAW_CONFIG))
        self.assertEqual('''
foo {
\tbar=42
\tbuzz=1
}''', bu._update_lvm_raw_config('foo', 'buzz', 1, RAW_CONFIG))
        self.assertEqual('''
foo {
\tbar=42
}
spam {
\tegg=1
}''', bu._update_lvm_raw_config('spam', 'egg', 1, RAW_CONFIG))
        self.assertEqual('''
foo {
\tbar=[1, 2.3, "foo", "buzz"]
}''', bu._update_lvm_raw_config('foo', 'bar', [1, 2.3, "foo", "buzz"],
                                RAW_CONFIG))

    @mock.patch.object(os, 'remove')
    @mock.patch.object(utils, 'execute')
    @mock.patch.object(bu, '_update_lvm_raw_config')
    @mock.patch.object(shutil, 'copy')
    @mock.patch.object(shutil, 'move')
    def test_override_config_value(self, m_move, m_copy, m_upd, m_execute,
        ...
        m_copy.assert_called_once_with('lvm.conf',
                                       'lvm.conf.bak')
        m_remove.assert_called_once_with('lvm.conf.bak')
