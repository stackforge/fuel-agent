    ...
    def get_os_by_image_meta(self):
        ...
        if os_release:
            ...
            try:
                ...
                os = OS(repos=None, packages=None)
                os.major = os_release['major']
                os.minor = os_release['minor']
                return os
            except (AttributeError, KeyError):
                LOG.warning('Cannot guess operating system release '
                            'from image metadata')

    def get_os_by_profile(self):
        LOG.debug('--- Getting operating system data by profile ---')
        profile = self.data['profile'].lower()
        if 'centos' in profile:
            os = objects.Centos(repos=None, packages=None)
            os.major = 6
            os.minor = 5
            if '7' in profile:
            ...
            os = objects.Ubuntu(repos=None, packages=None)
            os.major = 12
            os.minor = 4
            if '1404' in profile:
        ...
        os.major = os.minor = 'unknown'
        return os

    def parse_operating_system(self):
        LOG.debug('--- Preparing operating system data ---')
        return self.get_os_by_image_meta() or self.get_os_by_profile()
...
class NailgunBuildImage(BaseDataDriver):
    ...
    def parse_operating_system(self):
        ...
        os = objects.Ubuntu(repos=repos, packages=packages)
        os.major = 14
        os.minor = 4
        return os
