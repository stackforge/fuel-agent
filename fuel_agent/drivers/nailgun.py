...
class Nailgun(BaseDataDriver):
    ...
    def boot_disks(self):
        """
        Property to get suitable list of this to place '/boot'
        parition on it or choose user selected disks.
        ...
        if bootable_disk:
            if len(bootable_disk) >= 2:
                raise errors.WrongPartitionSchemeError(
                    "More than one bootable disk found! %{0}".format(bootable_disk))
