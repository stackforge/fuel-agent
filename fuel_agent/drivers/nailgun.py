...
class NailgunMkBootstrap(NailgunBuildImage):
    ...
    def __init__(self, data):
        ...
        ubuntu_pkgs_default = [
            ...
            "wget",
            "biosdevname",
            "linux-image%s" % (self.data.get('kernel_flavor',
                                             '-generic-lts-trusty')),
            "linux-firmware",
            "linux-firmware-nonfree",
            "xz-utils",
        ...
        self.data['packages'] = self.data.get('packages') or []
    ...
    def parse_schemes(self):
        ...
        if 'extend_kopts' not in self.data:
            ...
            (LOG.warning('Cannot find predetermined extend_kopts, so no new '
                      'parameteres will be passed to cobbler' ))
        ...
        m_kernel = {'uri': 'http://127.0.0.1:8080/bootstraps/{0}/vmlinuz'
                        .format(self.data['uuid'])}
        m_initrd = {'compress_format': 'xz',
                       'uri': 'http://127.0.0.1:8080/bootstraps/{0}/'
                              'initrd.img'.format(self.data['uuid'])}
        ...
        self.data['bootstrap_modules'] = { 'm_kernel' : m_kernel,
                                           'm_initrd' : m_initrd,
                                           'm_rootfs' : m_rootfs
        }
