...
class SimpleNailgunDriverValidator(object):
    schema = {
    ...
    def validate_pvs(self):
        partition_devices = set(prt.get('device') for prt in self.partitions)
        for pv in self.data.get('pvs', []):
            if pv['device'] not in partition_devices:
                self.raise_error(
                    "Physical Volume cannot be assigned to not existing "
                    "partition.\n'{0}' not in {1}".format(pv['device'],
                                                          partition_devices))
