...
def _restore_environ(varname, oldval):
    if oldval is None:
        try:
...
def get_ext234_superblock_timestamps(device):
    ...
    def tune2fs_strptime(string):
        ...
        try:
            ...
            _restore_environ('LC_ALL', old_LC_ALL)
...
def set_ext234_superblock_timestamps(device,
    ...
    LOG.debug('Set SB timestamps on %s: %s', device, debugfs_commands)
    try:
        ...
        LOG.debug('debugfs -f - -w %s failed: exit code %s',
                  device, e.exit_code)
        raise errors.FsUtilsError('Failed to run debugs %s, '
    ...
    LOG.debug('check if timestamps on %s has been actually set', device)
    ...
    if actual_timestamps != timestamps:
        ...
        raise errors.FsUtilsError(
            "Failed to set superblock timestamps of %s" % device)
...
def distant_past_ext234_superblock_timestamps(device):
    ...
    LOG.debug('Set SB timestamps of %s to a distant past', device)
