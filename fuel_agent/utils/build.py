...
def copy_kernel_initramfs(chroot, dstdir, clean=False, newest=False):
    ...
    for module in six.iterkeys(files):
        ...
        if len(all_files) > 1:
            LOG.debug('Multiply files found! But only one will be copyed!')
        if newest:
            file_to_copy = max(glob.iglob(mask), key=os.path.getctime)
            LOG.debug('Latest one by file-change-time file has beed chosen')
        else:
            file_to_copy = all_files[0]
            LOG.debug('Latest one by file-name-X-Y file has beed chosen')
