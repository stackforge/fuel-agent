...
def run_debootstrap(uri, suite, chroot, arch='amd64', eatmydata=False,
    ...
    if proxies:
        if 'http' in proxies:
            proxy = proxy + ['export http_proxy={0} ;'.format(proxies['http'])]
        if 'https' in proxies:
            proxy = proxy + ['export https_proxy={0} ;'.format(
                proxies['https'])]
        if 'ftp' in proxies:
            proxy = proxy + ['export ftp_proxy={0} ;'.format(proxies['ftp'])]
    cmds = [proxy, 'debootstrap', '--verbose', '--no-check-gpg',
            '--arch={0}'.format(arch)]
    if eatmydata:
...
def process_apt_sources(chroot, metadata, repos):
        metadata_ = metadata.copy()
        metadata_.setdefault('repos', [])
        try:
            for repo in repos:
                LOG.debug(
                ...
                metadata_.setdefault('repos', []).append({
                    'type': 'deb',
                    'name': repo.name,
                    'uri': repo.uri,
                    'suite': repo.suite,
            ...
            LOG.error('Failed to process apt sources', exc)
            raise


def dump_mkbootstrap_yaml(metadata, d_data, c_dir):
    ...
    drop_data = {'modules': {}}
    for module in d_data['bootstrap_modules']:
        fname = os.path.basename(d_data['bootstrap_modules']
                                 [module]['uri'])
        ...
        drop_data['modules'][module] = {
            ...
            'raw_size': raw_size,
            'file': fname,
            'uri': d_data['bootstrap_modules'][module]['uri']
        }

    drop_data['uuid'] = d_data['uuid']
    drop_data['os'] = metadata['os']
    drop_data['extend_kopts'] = d_data['extend_kopts']
    drop_data['all_packages'] = metadata['all_packages']
    drop_data['repos'] = d_data['repos']

    LOG.debug('Image metadata: %s', drop_data)
    with open(os.path.join(c_dir, d_data['meta_file']),
              'wt') as f:
