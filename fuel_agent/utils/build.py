...
def folder_to_archive(d_input, d_output, name, container):
    if container == 'tar.gz':
        o_file = os.path.join(os.path.normpath(d_output), name + container)
...
def propagate_host_resolv_conf(chroot):
    ...
    for conf_name in ('resolv.conf', 'hosts'):
        ...
        if os.path.isfile(full_conf_name):
            LOG.info('Propagate default {0} inside chroot'.
                     format(full_conf_name))
            try:
                shutil.copy(full_conf_name, full_conf_name + '.bak')
                shutil.copy(os.path.join('/etc/', conf_name), full_conf_name)
            except IOError:
                LOG.warning('Unable to pass default {0} inside chroot'.format(
                    conf_name))
                pass
...
def run_mksquashfs(chroot, dstdir, compression_algorithm='xz'):
    ...
    try:
        ...
        utils.execute(
            ...
            '-comp {0} -no-progress -noappend'.format(compression_algorithm),
            logged=True)
