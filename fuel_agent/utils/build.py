...
def run_mksquashfs(chroot, dstdir, compression_algorithm='xz'):
    ...
    try:
        ...
        for module in ('initrd', 'vmlinuz'):
            ...
            newest = os.path.join(boot_dir, max(
                filelist, key=lambda x: os.stat(
                    os.path.join(boot_dir, x)).st_mtime))
            LOG.debug('Moving out file: {0}'.format(newest))
            shutil.copy(newest, newest + temp)
        ...
        utils.execute(
            ...
            s_src.lstrip(chroot),
            os.path.join(s_dst.lstrip(chroot), files['squashfs']) + temp,
            '-comp {0}'.format(compression_algorithm),
            '-no-progress', '-noappend',
            logged=True)

        # move to result names
        # Move file with m_name* to m_name.
        # Example :
        # m_name = 'vmlinuz'
        # real-filename = 'vmlinuz-123-45'
        # result-file will be : 'vmlinuz'
        for m_name in six.iterkeys(files):
            f = filter(lambda x: os.path.join(s_dst, x)and x.startswith(
                m_name), os.listdir(s_dst))
            shutil.move(os.path.join(s_dst, f),
