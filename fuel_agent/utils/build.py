...
def copy_kernel_initramfs(chroot, dstdir, clean=False):
    """Copy latest vmlinuz and initrd from chroot
    ...
    for module in six.iterkeys(files):
        file_to_copy = max(glob.iglob(os.path.join(
            boot_dir, module + '*')), key=os.path.getctime)
        copy_to = os.path.join(dstdir, files[module])
        LOG.debug('Copying file: {0} to: {1}'.format(
            file_to_copy, copy_to))
...
def run_mksquashfs(chroot, dstdir, compression_algorithm='xz'):
    ...
    """
    # TODO(azvyagintsev) fetch name from uri driver
    squashfs_file = os.path.join(dstdir, 'root.squashfs')
    ...
    try:
        ...
        copy_kernel_initramfs(chroot, dstdir, clean=True)
        fu.mount_fs(
        ...
        # run mksquashfs
        squashfs_temp = '/mnt/dst/{0}'.format(temp)
        utils.execute(
            'chroot', chroot, 'mksquashfs', '/mnt/src/', squashfs_temp,
            '-comp {0}'.format(compression_algorithm),
            '-no-progress', '-noappend', logged=True)
        # move to result name
        LOG.debug('Moving file: {0} to: {1}'.format(
            os.path.join(chroot, squashfs_temp), squashfs_file))
        shutil.move(os.path.join(chroot, squashfs_temp), squashfs_file)
