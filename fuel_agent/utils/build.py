...
def copy_kernel_initramfs(chroot, dstdir, clean=False, newest=False):
    ...
    :param newest:
       if false- will try to find latest by file-name(version)
       if true- will try to find latest by file-change-time
    :return:
    """
    ...
    for module in six.iterkeys(files):
        ...
        if len(all_files) > 1:
            LOG.warning('Multiply files found!But only one will be copyed!')
        if newest:
            file_to_copy = max(glob.iglob(mask), key=os.path.getctime)
            LOG.warning('Latest one by file-change-time file has beed chosed')
        else:
            file_to_copy = all_files[0]
            LOG.warning('Latest one by file-name-X-Y file has beed chosed')
...
def run_mksquashfs(chroot, output_name=None, compression_algorithm='xz'):
    ...
    try:
        ...
        # move to result name
        LOG.debug('Moving file: {0} to: {1}'.format(long_squash, output_name))
