...
def propagate_host_resolv_conf(chroot):
    ...
    for dns_file in ('resolv.conf', 'hosts'):
        if os.path.isfile(os.path.join(c_etc, dns_file)):
            LOG.info('Propagate default {0} inside chroot'.
                     format(dns_file))
            shutil.copy(os.path.join(c_etc, dns_file),
                        os.path.join(c_etc, dns_file) + '.bup')
...
def restore_resolv_conf(chroot):
    ...
    utils.makedirs_if_not_exists(c_etc)
    for dns_file in 'resolv.conf' 'hosts':
        if os.path.isfile(os.path.join(c_etc, dns_file + '.bup')):
