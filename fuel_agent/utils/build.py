...
def make_targz(source_dir, output_name=None):
    ...
    or an absolute path
     """
    if not output_name:
        output_name = next(tempfile._get_candidate_names())
...
def run_mksquashfs(chroot, dstdir, compression_algorithm='xz'):
    ...
    try:
        ...
        files_to_copy = {'squashfs': files['squashfs']}
        for module in ('initrd', 'vmlinuz'):
            files_to_copy[module] = utils.guess_filename(
                path=boot_dir,
                regexp=(r'^{0}*'.format(module)))
            LOG.debug('Moving out file: {0} to: {1}'.format(
                files_to_copy[module], dstdir))
            shutil.copy(os.path.join(boot_dir, files_to_copy[module]),
                        os.path.join(dstdir, files_to_copy[module]) + temp)
        ...
        for module in six.iterkeys(files_to_copy):
                shutil.move(os.path.join(dstdir, files_to_copy[module] + temp),
