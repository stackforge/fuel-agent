...
def containerize(filename, container, chunk_size=1048576):
    if container == 'gzip':
        ...
        return output_file
    if container == None :
        LOG.info('File {0} not going to be containerized'.format(filename) )
        return filename
...
def folder_to_tar_gz(d_input, d_output, name):
    o_file = os.path.normpath(d_output) + '/' + name + '.tgz'
    LOG.info('Creating archive: %s', o_file)
    try:
        tar = tarfile.open( o_file, "w:gz" )
        for name in os.listdir( os.path.normpath(d_input) ):
            tar.add(os.path.normpath(d_input) + '/' + name)
        tar.close()
...
def run_script_in_chroot(chroot,script):
    ...
    LOG.info('Copy user-script {0} into chroot:{1}'
               .format(script,chroot))

    utils.execute('cp', '-r', script , chroot + '/')

    LOG.info('Make user-script {0} executable:'
               .format(script))
    utils.execute('chmod', '0755', chroot + '/' + os.path.basename(script) )

    stdout, stderr = utils.execute( 'chroot' , chroot , '/bin/bash', '-c',
...
def recompress_initramfs(chroot,compress):
    ...
    LOG.info('Change initramfs compression type to:{0}'
               .format(compress))
    utils.execute('sed', '-i', 's/COMPRESS\s*=\s*gzip/COMPRESS={0}/'.
                  format(compress), os.path.join(chroot, 'etc/initramfs-tools'
                                                         '/initramfs.conf'))

    stdout, stderr = utils.execute('find', os.path.join(chroot, 'boot/'),
                                '-iname','"initrd*"', '-exec', 'rm', '-vf',
                                '{} \;')

    LOG.info('Removing old initramfs completed: \nstdout:{0} '
              '\nstderr:{1}'.format(stdout,stderr))

    cmds = [ 'chroot', chroot, 'bash -c "''export TMPDIR=/tmp;', 'export TMP=/tmp;',
...
def populate_squashfs(chroot, compress, dstdir):
    ...
    try:
        utils.execute_logged('find', os.path.join(chroot, 'boot/'),
                                '-maxdepth 1', '-type', 'f',
                                '\( -iname', '"initrd*"',
                                '-o -iname', '"vmlinuz*" \)',
                                '-exec sh -c',
                                '\'cp -va {{}} {0}/`basename {{}}`.{1}\' \;'
                                .format(os.path.normcase(dstdir),temp))
        ...
        utils.execute_logged('chroot', chroot,
                             'mksquashfs',
                             '/mnt/src',
                             '/mnt/dst/{0}.{1}'.format(files['squashfs']
                                                       ,temp),
                             '-comp {0} -no-progress -noappend'
                             .format(compress))
        # move to result names
        for file in files.keys():
            # utils.execute_logged('mv -v',
            #                      s_dst + files[file] + '*',
            #                     os.path.normcase(dstdir) + '/'
            #                      + files[file])
            utils.execute_logged('find', s_dst,
                                '-maxdepth 1', '-type', 'f',
                                '-iname "{0}*"'.format(files[file]),
                                '-exec sh -c',
                                '" mv -v {{}} {0}/{1} " \;'
                                .format(os.path.normcase(dstdir),files[file]))
...
def dpkg_list(chroot):
    ...
    stdout, stderr = utils.execute('chroot', chroot, 'dpkg-query','-W -f=\'${Package} ${Version}\\t\' ')
    ...
    for i in range(len(p_list)):
        p_dict[p_list[i].split(' ')[0]] = p_list[i].split(' ')[1]
    return p_dict
