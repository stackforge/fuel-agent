...
def _update_lvm_raw_config(section, name, value, raw_config):
...
def override_lvm_config_value(chroot, section, name, value, lvm_conf_file):
    ...
    If option is not valid errors.ProcessExecutionError raises and
    lvm configuration keeps unchanged
    """
    updated_config = _update_lvm_raw_config(
        section, name, value,
        utils.execute('chroot', chroot, 'lvm dumpconfig')[0])

    shutil.copy(lvm_conf_file, lvm_conf_file+'.bak')
    ...
    try:
        ...
        shutil.move(lvm_conf_file+'.bak', lvm_conf_file)
        LOG.debug('Option {}/{} can not be updated on value {}. '
                  'lvm.conf restored'.format(section, name, value))
        raise exc
    else:
        os.remove(lvm_conf_file+'.bak')


def append_lvm_devices_filter(chroot, multipath_lvm_filter):
    """Append custom devises filters to LVM configuration

    LVM configuration should de updated to force lvm work with partions
    on multipath devices using /dev/mapper/<id>-part<n> links.
    Other links to these devices should be blacklisted in device/filter
    option
    """
    lvm_filter = set(get_lvm_config_value(chroot, 'devices', 'filter') or [])
    lvm_filter.update(multipath_lvm_filter)
    override_lvm_config_value(chroot, 'devices', 'filter',
                              sorted(list(lvm_filter)),
                              os.path.join(chroot, 'etc/lvm/lvm.conf'))
