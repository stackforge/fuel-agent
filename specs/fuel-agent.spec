...
install -d -m 755 %{buildroot}%{_sysconfdir}/fuel-agent
install -p -D -m 644 %{_builddir}/%{name}-%{version}/contrib/fuel_bootstrap/fuel_bootstrap_cli/fuel_bootstrap/settings.yaml.sample %{buildroot}%{_sysconfdir}/fuel-agent/fuel_bootstrap_cli.yaml
install -d -m 755 %{buildroot}%{_datadir}/fuel_bootstrap/files/
cp -a %{_builddir}/%{name}-%{version}/contrib/fuel_bootstrap/files/* %{buildroot}%{_datadir}/fuel_bootstrap/files/
...
%attr(0644,root,root) %config(noreplace) %{_datadir}/fuel_bootstrap/files/*
%attr(0755,root,root) %config(noreplace) %{_datadir}/fuel_bootstrap/files/trusty/usr/bin/fix-configs-on-startup
%attr(0755,root,root) %config(noreplace) %{_datadir}/fuel_bootstrap/files/trusty/usr/bin/send2syslog.py
%attr(0755,root,root) %config(noreplace) %{_datadir}/fuel_bootstrap/files/trusty/etc/rc.local
